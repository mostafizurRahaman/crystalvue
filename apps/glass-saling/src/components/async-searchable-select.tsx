...
export function AsyncSearchableSelect({
  ...
  const selectedLabel = React.useMemo(() => {
    console.log('AsyncSearchableSelect - selectedLabel calculation:', { value, options });
    const foundLabel = options.find((opt) => opt.value === value)?.label || value;
    console.log('AsyncSearchableSelect - foundLabel:', foundLabel);
    return foundLabel;
  }, [options, value]);
            ...
            {!loading && options.length > 0 && (
              <CommandGroup>
                {options.map((option) => (
                  <div
                    key={option.value}
                    className={`px-2 py-1.5 text-sm cursor-pointer hover:bg-accent hover:text-accent-foreground rounded-sm flex items-center ${value === option.value ? 'bg-accent text-accent-foreground' : ''}`}
                    onClick={() => {
